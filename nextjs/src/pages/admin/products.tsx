...
export default function ProductEdit({ products }: InferGetServerSidePropsType<typeof handler>) {
    ...
    const fieldNames: (keyof product & keyof dbEditReq['fields'])[] = ['path', 'name', 'description', 'price', 'manufacturer_link', 'img_link'];
        ...
        <Container className='my-5'>
            ...
            <Form onSubmit={handleSubmit}>
                <Form.Select defaultValue='part no' onChange={handleDropdownSelect}>
                    ...
                    {products.map((product) => <option key={product.part_no}>{product.part_no}</option>)}
                </Form.Select>
                {fieldNames.map((field, index) => (
                    <InputGroup className='mt-1' key={field}>
                        <InputGroup.Text style={{ width: '8em' }}>{field}</InputGroup.Text>
                        <InputGroup.Checkbox id={index} checked={enabledFields[index]} onChange={handleCheckChange} />
                        <Form.Control id={field} disabled={!enabledFields[index]} value={selectedProduct[field] || ''} onChange={handleFieldChange} />
                    </InputGroup>
                ))}
            ...
            <Form onSubmit={handleFileSubmit}>
                ...
                <Stack className='mt-2' direction='horizontal' gap={2}>
                    ...
                    <Button type='submit' disabled={confirm !== 'confirm'}>Upload CSV</Button>
                    <Form.Control
                        className='ms-auto'
                        style={{ maxWidth: '20em' }}
                        value={confirm}
