...
export default function Product(props: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    const { product: { path, part_no, name, description, price, img_link, manufacturer_link }, pid, structuredData } = props;
    return (
        <>
            <Head>
            ...
            <main className='container-fluid py-5 bg-light'>
                <div className='px-4 px-lg-5 d-flex align-items-center'>
                    <Link href={'/store' + path.replace(/(\/[^\/]+)$/, '').replaceAll(/\s/g, '-').toLowerCase()}>
                        <a>
                            <i role='button' className='bi bi-chevron-left mx-2' style={{ WebkitTextStroke: '2px' }} />
                        </a>
                    </Link>
                    <Breadcrumb className='mx-2 mb-0' activePath={path.split('/').filter(el => { return el != ''; })} pid={pid} />
                    <CloseButton className='ms-auto' onClick={() => router.back()} />
                </div>
                <div className='container px-4 px-lg-5 mt-3'>
                    <div className='row gx-4 gx-lg-5 align-items-center'>
                        <div className='col-md-6'>
                            <ImageWithFallback priority src={img_link} fallbackSrc={'/assets/no_img.svg'} alt={name} />
                        </div>
                        <div className='col-md-6'>
                            <div className='small mb-1'>Part No.: {part_no}</div>
                            <h1 className='display-5 fw-bolder'>{name}</h1>
                            <div className='fs-5 mb-5'>
                                <span>{Intl.NumberFormat('en-EG', { style: 'currency', currency: 'EGP' }).format(price)}</span>
                            </div>
                            <p className='text-muted text-truncate' style={{ textDecoration: 'underline', textDecorationStyle: 'dotted' }}>
                                <a href={manufacturer_link}><small>{manufacturer_link}</small></a>
                            </p>
